...
register only once on websites, not over and over again on each
...
database (see the section
xref:saving-sessions-in-the-database["Saving Sessions in the Database"]).
An independent and unique session ID is created automatically and the
